...
const done = [];
// Playbook + start-here + team documents
for (const f of fs.readdirSync(path.join(ROOT, 'docs/sdc-engineer'))) done.push(copy(path.join(ROOT, 'docs/sdc-engineer', f), path.join(SHARE, 'Playbook', f)));
...
for (const f of ['buildReadiness.cjs', 'mergeLessons.cjs', 'syncSharedFolder.cjs', 'html2docx.ps1', 'experiments/extractSession.cjs', 'experiments/splitL5x.cjs']) {
