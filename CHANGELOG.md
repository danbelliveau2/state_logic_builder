...
## Unreleased
- THE BUILD PLAN (Dan, 2026-09-03): the station page is three stages — INPUTS → BUILD PLAN (one page per station, one section per machine: design owner [yours vs standard SDC machine from shipped work + the values he supplies], devices, sequence-as-states, branches/retries, handshakes, initialization, standards + decisions + open questions) → SEQUENCE (v3 canvas per machine, entered by choice: Use first pass | Build your own) → Build. Chat edits the plan before approval (reflex + deep lane) and the document REDLINES (removed struck, added highlighted, ✓ got it clears). Plan derived from the sheet (buildPlan.mjs) + overlay (draft.plan); codegen reads the plan + the canvas when authoritative; header shows "v3 · Station Sheet · x.y.z"; the seventeen-step walk UI retired; four new layout invariants
- Fixes: two tabs on one draft no longer ping-pong autosaves (identical adoptions are no-ops); value.set supports dotted paths (delays.extendMs) and drops stray literal keys
