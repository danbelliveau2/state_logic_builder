...
1a. Deviations: `node "X:\Electrical Dept\SDC Engineer\Scripts\logDeviation.cjs" --list --job <n>`. Code that carries a Denied row is a finding (fix = the standard form). An Open or Approved row is not a finding; a departure that has no row is (fix = log it, or build the standard form).
