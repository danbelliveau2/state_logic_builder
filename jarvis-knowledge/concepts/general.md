...
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBaseEscapement [tuition]) When adapting a template to a station whose devices are RENAMES of the template's devices, the tag rename is only half the job — the state numbers wired into every R03 command rung, R01 conditioning rung, R02 transition, init branch, and alarm rung still belong to the template's sequence. Before writing, build an explicit two-column map (template state -> this station's state, template device -> this station's device) and walk EVERY routine against it. A plan that renames tags and rewrites message strings but leaves R02/R03 rung bodies untouched produces a program where no state commands the device the diagram assigns it: the file imports, and every action is at the wrong state.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBaseEscapement [tuition]) Every diagram action row is a contract for a specific rung pair: an R03 output rung whose condition list contains that state's Status.State[n], and an R02 transition rung out of that state gated on the device's completion bit. Before writing any plan, enumerate the action rows as a checklist (state number, device, direction) and confirm the plan contains an operation that puts that state into that device's command rung. Actions with no owning state, and states commanding a device the diagram never assigns them (the mirror defect), are both caught by walking this list rather than by reading the finished rungs.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBaseEscapement [tuition]) Template rung comments are state-specific prose ('Extend z axis', 'Wait for index complete') and are therefore stale the instant the station's state map differs from the template's. Rung comments are GENERATED from the diagram state map, never inherited: every R02 sequence rung the plan touches — and every one it does not — needs its comment reissued from the state label. Treat 'did I re-comment all sequence rungs' as a mandatory sweep, not a per-rung judgment, because an untouched rung silently keeps the template's wrong label.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBasePickAndPlace [tuition]) removeTag has a known collateral-damage radius: deleting one tag can take an adjacent declaration with it. The pre-write pass must therefore treat every removeTag as requiring a re-declare check — after listing the removals, walk the remaining rung text for any tag that the plan never removes but also never declares, and add an addTag for it. Cheaper still: only remove tags that belong to a genuinely absent DEVICE (its axis, routine, HMI and iq_ tags), and leave unreferenced scalars the template ships (part-present inputs, ONS helper bits) alone — a spare declared BOOL costs nothing, while its accidental loss costs a whole validation round.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBasePickAndPlace [tuition]) Ordering matters inside an edit plan: every destructive operation should be planned as a removal BLOCK placed last, with any re-declarations (addTag) after it, so a collateral deletion cannot silently undo an earlier addTag. Planning removals mid-plan and additions before them makes the outcome depend on operation order rather than on intent.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBasePickAndPlace [tuition]) When adapting a template to a station that lacks one of the template's devices, enumerate the device's own tag family explicitly (routine, axis, HMI, motion instances, RangeCheck instances, alarm slots, message strings) and delete only that family. Removals aimed at tidying up unrelated leftovers — a sensor input the new station does not use, a spare timer — are outside the device boundary and are where blast-radius defects come from.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBaseEscapement [tuition]) A plan that renames a template tag and a plan that references a new name are two different operations, and the merge engine will not infer one from the other. Before writing any rung, build the complete identifier ledger for the station: every tag name the new rungs will mention, tagged as (a) template tag kept as-is, (b) template tag renamed (needs an explicit renameTag), or (c) new tag (needs an explicit addTag). Any identifier in the rung text that is not in column (a), (b), or (c) is an undeclared-identifier error at merge time. Doing this ledger pass first is what turns device renaming from a per-rung guess into a checklist.
- (2026-08-31, from the fix loop (tuition)'s correction of build PNP_ServoX- PneumaticZ / MidBaseEscapement [tuition]) When a station's devices are renames of the template's devices, the derived status/command/timer families rename with them — not just the physical i_/q_ parameters. The template's conditioned status bits (XAxisExtended, GripperClosed), manual-permissive helpers (OkManExtendX), debounce and alarm-timer instances all carry the old device's name and are all referenced by the new rungs. Enumerate the FULL tag family per device (physical inputs, physical outputs, derived status bits, delay timers, alarm timers, debounce instances, manual helper bits) at plan time and decide rename-vs-add for each; touching only the i_/q_ layer leaves every derived reference undeclared.
