...
async function generateL5X(projectJson, smId, options = {}) {
  ...
  const modelEffort = mode === 'translation' ? 'medium' : undefined;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (parsed.plan && parsed.plan.toBeContinued === true && Array.isArray(parsed.plan.operations)) {
      ...
      while (chunk && chunk.toBeContinued === true) {
        ...
        const cont = await callModel(client, system, messages, { signal: abortSignal, effort: modelEffort });
      ...
      if (error) { stage = 'parse'; feedback = `Your response was not a valid JSON edit plan. ${error}. Respond with ONLY the JSON object.`; }
        ...
        if (!schema.ok) {
          ...
          if (!feedback) {
            ...
            // in the compiled IR, so the cross-check runs against it.
            const d = (mode === 'translation' && compiledIr)
              ? validateAgainstCompiledIR(compiledIr, l5x)
