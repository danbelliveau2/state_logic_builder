...
function fallbackHelpQuestions(persistentFindings, rounds, smName) {
  return (persistentFindings || []).slice(0, 6).map(f => ({
    question: `Generation of ${smName} is held: this finding survived ${rounds} fix round(s) and the SDC Engineer could not clear it — "${String(f).slice(0, 300)}". How should it be resolved?`,
    ...
    for (const q of questions) {
      if (!isRealQuestion(q.question)) {
        decisions.push(`Filed as a statement, kept as a note (statements are not questions): ${q.question}` +
          (q.proposedSolution ? ` — SDC Engineer's take: ${q.proposedSolution}` : ''));
      }
    }
...
async function generateL5X(projectJson, smId, options = {}) {
  ...
  const stamp = `Generated by SDC Engineer v${JARVIS_VERSION} (${currentEntry().date}) - SDC State Logic Builder`;
  ...
  if (options.resume && options.resume.lastEditPlan) {
    ...
    messages.push({
      ...
      content: [
        ...
        ...(r.answers || []).flatMap(a => [
          `Q: ${a.question}`,
          `SDC Engineer proposed: ${a.proposedSolution || '(no proposal)'}`,
  ...
  if (reviewEnabled && validation.ok && l5x) {
    onProgress(94, 'review', 'Internal review — SDC Engineer checking the file against the template like the senior CE would');
    try {
