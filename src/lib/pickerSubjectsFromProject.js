...
export function getProjectSubjects(project, smId) {
  ...
  // ── Per-SM station-result fields ──────────────────────────────────────
  // Each SM auto-derives a `S{NN}_PartPass` BOOL field (rolled up from
  // every Check in the SM). Surface them as decision subjects across the
  // whole project so downstream stations can say "If S03_PartPass = false
  // → divert to reject chute". The fields are stable — generated by SM
  // station number, no need to manually maintain them.
  if (Array.isArray(project.stateMachines)) {
    for (const otherSm of project.stateMachines) {
      const station = `S${String(otherSm.stationNumber ?? 0).padStart(2, '0')}`;
      const fieldName = `${station}_PartPass`;
      subjects.push({
        id: `__stationPass_${otherSm.id}`,
        name: fieldName,
        grammarRowId: 'partTracking',
        detailValues: {},
      });
